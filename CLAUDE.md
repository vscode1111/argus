...
- Pluralization: `plural(count, singular, pluralForm?)` in `webview/src/utils/text.ts` for UI labels (e.g. "1 file" / "3 files"); inlined copy in `argusServer.ts` (can't share .ts imports across tsconfig boundaries)
