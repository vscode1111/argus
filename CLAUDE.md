...
- Webview message protocol (extension -> webview): typed as `WebviewMessage` union in `ChatPanel.ts` - `thinking_start | thinking_chunk | text_chunk | tool_start | tool_end | done | error | message | clear | prefill | skills | workspaceInfo | log | clearLogs | loginUrl | loginResult | contextUsage | filePreview | retry_status`
...
- Retry: server stores `lastMessage`; webview sends `retry` message which the server re-emits as a `send`; error block Retry button hides the error block on click via local state; watchdog-retried messages show an inline Retry button (`.retryBtn` in `ChatMessage.module.css`) that hides on click
