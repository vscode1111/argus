...
const CASES = [
  ...
  },
  {
    name: 'count-gate',
    file: f('src', 'backend', 'cliHandler.ts'),
    from: '    if (isBackgroundTool(s, event.tool_use_id)) addBgTask(s, event.task_id as string);',
    to: '    addBgTask(s, event.task_id as string);',
    compile: true,
    grep: 'a plain Bash call is not counted',
    why: 'every Bash call counts as a background task again, so the pill blinks',
  },
  {
    name: 'auto-background',
    file: f('src', 'backend', 'cliHandler.ts'),
    // No trailing newline in the anchor: this file is CRLF, so `;\n` matches nothing here.
    from: '  noteBackgroundLaunch(s, result);',
    to: '  void result;',
    compile: true,
    grep: 'backgrounds by itself is counted',
    why: 'a command the CLI backgrounds on its own stops being counted at all',
  },
  {
    name: 'marker-idle-gate',
    file: f('src', 'backend', 'cliHandler.ts'),
    from: '    const notice = s.cliDone ? noticeFromSystemEvent(event) : null;',
    to: '    const notice = noticeFromSystemEvent(event);',
    compile: true,
    grep: 'raised for a turn nobody asked for',
    why: 'the cause marker announces every finished command, echo included',
  },
  {
    name: 'timer-colour',
