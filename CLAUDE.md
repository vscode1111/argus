...
- Retry: server stores `lastMessage`; webview sends `retry` message; server sends `retry_clean` (reducer removes trailing error + error-outcome assistant messages) then re-emits as `send` with `_silent: true` (no duplicate user message); `thinking_start` is always sent regardless of `_silent` so streaming state initializes; watchdog-retried messages show an inline Retry button (`.retryBtn` in `ChatMessage.module.css`) that hides on click; e2e tests in `e2e/retry-clean.spec.ts`
